function PlotRecover(xo,x,pos,ind)

    figure('Renderer', 'painters', 'Position', pos)
    axes('Position', [0.05 0.1 0.9 0.8] );
    stem(find(xo),xo(xo~=0),'o-','Color','#f26419','MarkerSize',7, 'LineWidth',1),hold on
    stem(find(x),x(x~=0),'o:', 'filled','Color','#1c8ddb', 'MarkerSize',4, 'LineWidth',1),hold on
